...
test.beforeEach(async ({ page }) => {
  let encodedAuthCredentials = Buffer.from(`${process.env.SEP_QA_USERNAME}:${process.env.SEP_QA_PASSWORD}`).toString('base64');
...
test('Product Landing Page', async ({ page }) => {
  ...
  const cookiePolicyLink = page.locator(
    "//a[contains(text(),'Cookie Policy')]"
  );
